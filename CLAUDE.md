...
- El sitio está publicado como Artifact de claude.ai: https://claude.ai/artifact/CmToMKGB5f1CDha4Ff1gTX. Está compartido con cualquiera que tenga el enlace y los cambios se ven al instante.
...
- **Actualización automática semanal:** la tarea programada "Actualizar Bitácora de Opiniones OECE" (`actualizar-bitacora-oece`, lunes a las 9:00) ejecuta `python scripts/gobpe.py actualizar`, clasifica lo nuevo, corre `build.py` y republica. Corre en la PC del autor mientras la app de Claude está abierta (si está cerrada, al abrirla).
